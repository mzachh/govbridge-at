...
function safeDestination(form: HTMLFormElement, submitter: HTMLElement, pageUrl: string, expectedPath: string): boolean {
  ...
  try {
    for (const action of actions) {
      ...
      if (url.origin !== "https://www.meinesv.at" || url.pathname !== expectedPath ||
...
function inspectForm(pageDocument: Document): SearchForm | undefined {
  ...
  const expectedHeading = typeRoute ? "Einreichungen abfragen" : "Liste der Einreichungen";
  ...
  if (typeRoute) {
    const tabs = Array.from(form.querySelectorAll<HTMLElement>('[role="tab"]'))
      .filter((tab) => text(tab) === "Wahlarzt / Wahltherapeut");
  ...
  const expectedSubmitterText = typeRoute ? "Weiter" : "OK";
  ...
  if (submitters.length !== 1 || !safeDestination(form, submitters[0]!, pageDocument.URL, expectedPath)) return undefined;
...
export async function searchClaims(pageDocument: Document, input: unknown): Promise<SearchResult> {
  ...
  if (!isSearchToolUrl(pageDocument.URL)) {
    return isSearchToolUrl(pageDocument.URL) ? failure("FORM_UNAVAILABLE") : failure("UNSUPPORTED_PAGE");
  }
